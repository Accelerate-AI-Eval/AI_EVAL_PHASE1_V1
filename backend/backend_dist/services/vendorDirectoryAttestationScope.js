...
export function attestationBelongsToVendorDirectoryRow() {
    return or(eq(vendorSelfAttestations.user_id, vendors.userId), eq(vendorSelfAttestations.organization_id, vendors.organizationId), eq(vendorSelfAttestations.organization_id, createOrganization.organizationName));
}
/** Same rules as {@link attestationBelongsToVendorDirectoryRow} using resolved vendor row values (no join). */
export function attestationBelongsToVendorByIds(vendorUserId, vendorOrganizationId, organizationName) {
    const parts = [];
    ...
    if (oid)
        parts.push(eq(vendorSelfAttestations.organization_id, oid));
    if (oname && oname !== oid)
        parts.push(eq(vendorSelfAttestations.organization_id, oname));
    if (parts.length === 0) {
        return sql `false`;
    }
    if (parts.length === 1)
 ...
 */
export async function enablePublicDirectoryListingForAttestation(attestationId) {
    const [row] = await db
        .select({
        ...
        .limit(1);
    if (row?.user_id == null || !Number.isInteger(Number(row.user_id)))
        return;
    const uid = Number(row.user_id);
    const orgId = row.organization_id != null ? String(row.organization_id).trim() : "";
    const whereClause = orgId !== "" ? or(eq(vendors.userId, uid), eq(vendors.organizationId, orgId)) : eq(vendors.userId, uid);
