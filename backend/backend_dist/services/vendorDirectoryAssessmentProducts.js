...
export async function resolveProductByOrgAndProductName(vendorName, productName) {
    ...
    const [row] = await db
        ...
        .where(and(sql `trim(lower(${createOrganization.organizationName})) = trim(lower(${vName}))`, sql `trim(lower(coalesce(${vendorSelfAttestations.product_name}, ''))) = trim(lower(${pName}))`, sql `upper(trim(coalesce(${vendorSelfAttestations.status}, ''))) = 'COMPLETED'`))
...
export async function resolveProductByAttestationForVendorUser(vendorUserId, attestationRef) {
    ...
    const [row] = await db
        ...
        .where(and(eq(vendorSelfAttestations.user_id, vendorUserId), or(eq(vendorSelfAttestations.id, ref), eq(vendorSelfAttestations.vendor_self_attestation_id, ref)), sql `upper(trim(coalesce(${vendorSelfAttestations.status}, ''))) = 'COMPLETED'`))
        .limit(1);
    if (!row?.attestationId)
        return null;
    return { ...row, attestationId: String(row.attestationId) };
