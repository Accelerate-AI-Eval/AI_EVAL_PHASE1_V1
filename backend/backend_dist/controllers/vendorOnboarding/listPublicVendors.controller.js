    ...
    try {
        ...
        const productRows = vendorIds.length > 0
            ? await db
                ...
                .where(and(inArray(vendorSelfAttestations.user_id, vendorIds), eq(vendorSelfAttestations.visible_to_buyer, true), sql `upper(${vendorSelfAttestations.status}) = 'COMPLETED'`, sql `(${vendorSelfAttestations.expiry_at} IS NULL OR ${vendorSelfAttestations.expiry_at} >= now())`))
            : [];
