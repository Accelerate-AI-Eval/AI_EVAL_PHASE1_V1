...
const listVendorVisibleProducts = async (req, res) => {
    try {
        ...
        const directoryAttestationJoin = and(attestationBelongsToVendorDirectoryRow(), isNull(vendorSelfAttestations.user_archived_at));
        const rows = allProducts && isSystemAdmin
            ...
            : await db
                ...
                .from(vendors)
                .innerJoin(createOrganization, vendorOrgJoin)
                .innerJoin(vendorSelfAttestations, and(directoryAttestationJoin, eq(vendorSelfAttestations.status, "COMPLETED"), eq(vendorSelfAttestations.visible_to_buyer, true), sql `(${vendorSelfAttestations.expiry_at} IS NULL OR ${vendorSelfAttestations.expiry_at} >= now())`))
                .where(eq(vendors.id, vendorId))
                .orderBy(desc(vendorSelfAttestations.updated_at));
