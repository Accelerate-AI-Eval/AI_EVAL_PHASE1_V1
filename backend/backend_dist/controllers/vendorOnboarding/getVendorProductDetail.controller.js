    ...
    try {
        ...
        if (allParam && isSystemAdmin) {
            ...
            const [r] = await db
                .select()
                .from(vendorSelfAttestations)
                .where(and(eq(vendorSelfAttestations.id, productId), eq(vendorSelfAttestations.user_id, vendorUserId), eq(vendorSelfAttestations.status, "COMPLETED"), eq(vendorSelfAttestations.visible_to_buyer, true), sql `(${vendorSelfAttestations.expiry_at} IS NULL OR ${vendorSelfAttestations.expiry_at} >= now())`))
                .limit(1);
            row = r;
        }
        if (!row &&
            ...
            const [r] = await db
                .select()
                .from(vendorSelfAttestations)
                .where(and(eq(vendorSelfAttestations.id, productId), eq(vendorSelfAttestations.user_id, vendorUserId), eq(vendorSelfAttestations.status, "COMPLETED")))
                .limit(1);
            row = r;
        }
        if (!row) {
        ...
        const rowRecord = row;
        const attestationIdForReport = rowRecord.id != null && String(rowRecord.id).trim() !== "" ? String(rowRecord.id) : productId;
        ...
        const generatedProfileReport = rawReport != null ? mergeSummaryIntoReport(rawReport, reportRow?.summary) : undefined;
