 ...
 */
const listAssessmentsByOrganization = async (req, res) => {
    try {
        ...
        const platformRole = String(user.user_platform_role ?? "").trim().toLowerCase().replace(/_/g, " ");
      ...
      WHERE ${whereClause}
      ORDER BY a.created_at DESC`, queryParams);
        const list = rows.map((r) => ({
            ...
            productName: r.type === "cots_vendor"
            ...
            reportRiskScore: r.reportRiskScore != null && Number.isFinite(Number(r.reportRiskScore))
        ...
        if (detail)
            console.error("listAssessmentsByOrganization: Detail:", detail);
        if (code)
            console.error("listAssessmentsByOrganization: Code:", code);
